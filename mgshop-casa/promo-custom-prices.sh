...
cat > "components/admin/promo-manager.tsx" << 'PROMOEOF'
"use client"
...
export function PromoManager() {
  ...
  const itemProducts = items
    ...
    .filter((x): x is { item: PromoItem; product: Product } => !!x.product)
...
mkdir -p "app/promo"
cat > "app/promo/page.tsx" << 'PROMOEOF'
"use client"
import { useState, useEffect } from 'react'
import Link from 'next/link'
...
git add -A
git commit -m "feat: prezzo originale e prezzo promo personalizzabile per ogni prodotto in Promo"
git push
