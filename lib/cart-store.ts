...
import type { CartItem, CartSource, CustomizationSelection, Product } from './types'
import { buildCartLineId } from './customization'

interface CartStore {
  items: CartItem[]
  lastAdded: number
  addItem: (product: Product, customization?: CustomizationSelection[], unitPrice?: number, source?: CartSource) => void
...
export const useCartStore = create<CartStore>()(
  persist(
    (set, get) => ({
      items: [],
      lastAdded: 0,
      addItem: (product, customization, unitPrice, source) => {
        ...
        const newItems = existing
          ...
          : [...items, { product, quantity: 1, customization, lineId, unitPrice, source }]
