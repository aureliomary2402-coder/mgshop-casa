...
export function HeroBanner({ banners, categories }: { banners: Banner[]; categories: Category[] }) {
  ...
  return (
    <section className="relative overflow-hidden" style={{ background: 'linear-gradient(120deg, #0c2b36 0%, #0891b2 55%, #06b6d4 100%)' }}>
      ...
      <div className="hidden md:block absolute right-4 lg:right-10 top-1/2 -translate-y-1/2 z-[5] pointer-events-none animate-float-slow">
        <Image
          src="/logo/mgshop-logo-neon.png"
          alt="MGShop Casa"
          width={340}
          height={340}
          className="neon-glow-logo w-40 h-40 lg:w-56 lg:h-56 xl:w-64 xl:h-64 object-contain"
          priority
        />
      </div>
