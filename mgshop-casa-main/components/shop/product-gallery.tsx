  ...
  if (images.length === 0) {
    return (
      <div className="relative aspect-square rounded-3xl overflow-hidden mb-3 flex items-center justify-center"
        style={{ background: 'linear-gradient(135deg, #EDF5FD, #D2E6FB)', boxShadow: '0 20px 60px rgba(8,145,178,0.12)' }}>
    ...
    <>
      <div>
        <div
          ...
          style={{
            background: 'linear-gradient(135deg, #EDF5FD, #D2E6FB)',
          ...
          {images.length > 1 && (
            <>
              <button onClick={(e) => { e.stopPropagation(); goPrev() }}
                ...
                <ChevronLeft className="w-5 h-5" style={{ color: '#0B3C65' }} />
              </button>
              <button onClick={(e) => { e.stopPropagation(); goNext() }}
                ...
                style={{ background: 'rgba(240,251,253,0.9)', backdropFilter: 'blur(8px)' }}>
                <ChevronRight className="w-5 h-5" style={{ color: '#0B3C65' }} />
              </button>
              <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5 sm:hidden">
                {images.map((_, i) => (
                  <div key={i} className="w-1.5 h-1.5 rounded-full transition-all"
                    style={{ background: i === currentImg ? '#2578A4' : 'rgba(8,145,178,0.25)' }} />
                ))}
              </div>
            </>
          )}
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
            {images.map((img, i) => (
              <button key={img.id} onClick={() => setCurrentImg(i)}
                ...
                style={{ border: i === currentImg ? '2px solid #2578A4' : '2px solid transparent' }}>
