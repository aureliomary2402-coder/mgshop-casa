...
export function ProductCard({ product, index = 0 }: { product: Product; index?: number }) {
  ...
  return (
    ...
    >
      <div className="relative aspect-square overflow-hidden" style={{ background: 'linear-gradient(135deg, #EDF5FD, #D2E6FB)' }}>
        {imgUrl && !imgError ? (
          <img
            src={imgUrl}
            alt={product.name}
      ...
      <div className="p-3">
        <h3 className="font-semibold text-sm line-clamp-2 mb-2 leading-snug transition-colors"
          style={{ color: isHovered ? '#0B3C65' : '#041C33' }}>
          {product.name}
        </h3>
        <div className="flex items-center justify-between">
          <span className="font-bold text-base" style={{ color: '#2578A4' }}>€{product.price.toFixed(2)}</span>
          <button
