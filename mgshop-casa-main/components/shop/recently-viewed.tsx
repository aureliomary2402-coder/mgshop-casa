...
export function RecentlyViewed({ excludeId, title = 'Visti di recente' }: { excludeId?: string; title?: string }) {
  ...
  return (
    <div>
      <p className="text-sm font-semibold flex items-center gap-1.5 mb-2.5" style={{ color: '#041C33' }}>
          ...
          return (
            <button key={p.id} onClick={() => openDetail(p.id)}
              ...
              <div className="w-28 h-28 rounded-2xl overflow-hidden mb-1.5 transition-transform group-hover:scale-105"
                style={{ background: 'linear-gradient(135deg, #EDF5FD, #D2E6FB)', border: '1px solid rgba(8,145,178,0.08)' }}>
                {imgUrl && <img src={imgUrl} alt={p.name} className="w-full h-full object-cover" />}
              </div>
              <p className="text-xs font-medium line-clamp-2 leading-snug" style={{ color: '#041C33' }}>{p.name}</p>
              <p className="text-xs font-bold" style={{ color: '#2578A4' }}>€{p.price.toFixed(2)}</p>
            </button>
          )
        })}
      </div>
