...
function HighlightedText({ text, query }: { text: string; query: string }) {
  ...
  return (
    <>
      {text.slice(0, idx)}
      <span style={{ color: '#2578A4', fontWeight: 700 }}>{text.slice(idx, idx + query.length)}</span>
    ...
    <header className={`sticky top-0 z-50 transition-all duration-300 ${scrolled ? 'shadow-lg' : ''}`}
      ...
      <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between gap-3">
        <Link href="/" className="flex items-center gap-2 shrink-0 group">
          ...
          <span className="text-lg font-bold tracking-tight hidden sm:block" style={{ color: '#041C33' }}>
            MG<span style={{ color: '#2578A4' }}>Shop</span>
          </span>
        </Link>
        ...
        <div className="flex items-center gap-1">
          <div className="relative" ref={catRef}>
            <button onClick={() => setCatOpen(v => !v)}
              ...
              style={{ color: catOpen || activeCategory ? '#2578A4' : '#44403c' }}>
          ...
          {promoActive && (
            <Link href="/promo" className={`${searchOpen ? 'hidden' : 'flex'} items-center gap-1.5 px-2 sm:px-3 py-2 rounded-xl text-sm font-medium transition-all hover:bg-cyan-50 btn-press`} style={{ color: '#2578A4' }}>
          ...
          {volantinoActive && (
            <Link href="/volantino" className={`${searchOpen ? 'hidden' : 'flex'} items-center gap-1.5 px-2 sm:px-3 py-2 rounded-xl text-sm font-medium transition-all hover:bg-cyan-50 btn-press`} style={{ color: '#2578A4' }}>
                  ...
                  {searchResults.length > 0 ? (
                    <>
                        ...
                        {searchResults.map(product => {
                          ...
                          return (
                            <button key={product.id} onClick={() => handleSelectProduct(product)}
                              ...
                              <span className="flex-1 min-w-0 text-sm font-medium truncate" style={{ color: '#041C33' }}>
                                <HighlightedText text={product.name} query={searchValue} />
                              </span>
                              <span className="text-sm font-bold shrink-0" style={{ color: '#2578A4' }}>€{product.price.toFixed(2)}</span>
                            </button>
                          )
                        })}
                      </div>
                      <button onClick={handleSeeAllResults}
                        ...
                        style={{ borderColor: 'rgba(8,145,178,0.12)', color: '#2578A4' }}>
