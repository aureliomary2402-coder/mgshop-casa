...
export function CartContent({ scope = 'shop' }: { scope?: string }) {
  ...
  if (submitted) return (
    <div className="max-w-md mx-auto px-4 py-20 text-center animate-scale-in">
      <div className="relative w-24 h-24 mx-auto mb-6">
        {['#2578A4','#97C3EE','#97C3EE','#16a34a','#ef4444','#3b82f6'].map((color, i) => (
        ...
        <div className="w-24 h-24 rounded-full flex items-center justify-center animate-check-pop" style={{background:'linear-gradient(135deg,#2578A4,#97C3EE)',boxShadow:'0 20px 40px rgba(8,145,178,0.3)'}}>
          <CheckCircle className="w-12 h-12 text-white"/>
        </div>
      </div>
      <h2 className="text-3xl font-bold mb-3" style={{color:'#041C33'}}>Ordine inviato!</h2>
      ...
      <Link href="/shop" className="inline-flex items-center gap-2 font-bold px-8 py-4 rounded-2xl text-white btn-press" style={{background:'linear-gradient(135deg,#2578A4,#97C3EE)'}}>
  ...
  if (items.length === 0) return (
    <div className="max-w-md mx-auto px-4 py-20 text-center animate-scale-in">
      ...
      <h2 className="text-2xl font-bold mb-2" style={{color:'#041C33'}}>Il carrello è vuoto</h2>
      <p className="text-slate-400 mb-8">Aggiungi qualche prodotto per iniziare.</p>
      <Link href="/shop" className="inline-flex items-center gap-2 font-bold px-8 py-4 rounded-2xl text-white btn-press" style={{background:'linear-gradient(135deg,#2578A4,#97C3EE)'}}>Vai ai prodotti</Link>
  ...
  return (
    <div className="w-full overflow-x-hidden max-w-5xl mx-auto px-4 py-6 sm:py-8">
      <Link href="/shop" className="inline-flex items-center gap-2 text-sm font-medium mb-4 sm:mb-6 group transition-all hover:gap-3" style={{color:'#0B3C65'}}>
        <ArrowLeft className="w-4 h-4 group-hover:-translate-x-1 transition-transform"/> Continua lo shopping
      </Link>
      <h1 className="text-xl sm:text-2xl font-bold mb-5 sm:mb-8" style={{color:'#041C33'}}>
      ...
      <div className="grid md:grid-cols-3 gap-5 md:gap-6">
        <div className="md:col-span-2 space-y-3 min-w-0">
          {items.map(({product,quantity,lineId},i) => {
            ...
            <div key={rowId} className="flex gap-3 sm:gap-4 rounded-2xl p-3 sm:p-4 animate-fade-in-up"
              ...
              <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-xl overflow-hidden shrink-0" style={{background:'linear-gradient(135deg,#EDF5FD,#D2E6FB)'}}>
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm sm:text-base font-semibold truncate mb-1" style={{color:'#041C33'}}>{product.name}</p>
                <p className="font-bold" style={{color:'#2578A4'}}>€{product.price.toFixed(2)}</p>
                <div className="flex items-center gap-2 mt-2">
                  <button onClick={() => updateQuantity(rowId,quantity-1)} className="w-7 h-7 rounded-full flex items-center justify-center transition-all hover:scale-110 btn-press shrink-0" style={{border:'1px solid rgba(8,145,178,0.2)',background:'rgba(8,145,178,0.04)'}}><Minus className="w-3 h-3" style={{color:'#0B3C65'}}/></button>
                  <span className="w-6 text-center text-sm font-bold shrink-0" style={{color:'#041C33'}}>{quantity}</span>
                  <button onClick={() => updateQuantity(rowId,quantity+1)} className="w-7 h-7 rounded-full flex items-center justify-center transition-all hover:scale-110 btn-press shrink-0" style={{border:'1px solid rgba(8,145,178,0.2)',background:'rgba(8,145,178,0.04)'}}><Plus className="w-3 h-3" style={{color:'#0B3C65'}}/></button>
        ...
        <div className="rounded-2xl p-4 sm:p-5 h-fit md:sticky md:top-20 animate-slide-in-right space-y-4 min-w-0 glass-card">
          <h2 className="font-bold" style={{color:'#041C33'}}>Riepilogo ordine</h2>
                ...
                <div className="flex flex-wrap gap-2">
                  <div className="relative flex-1 min-w-[140px]">
                    ...
                    <input value={couponInput} onChange={e => setCouponInput(e.target.value.toUpperCase())} onKeyDown={e => e.key==='Enter'&&handleApplyCoupon()} placeholder="Codice coupon"
                      className="w-full h-10 pl-9 pr-3 rounded-xl text-base font-mono outline-none" style={{background:'rgba(8,145,178,0.05)',border:'1px solid rgba(8,145,178,0.15)',color:'#041C33'}}/>
                  </div>
                  <button onClick={handleApplyCoupon} disabled={couponLoading||!couponInput.trim()} className="shrink-0 px-3 py-2 rounded-xl text-sm font-medium text-white disabled:opacity-50 transition-all hover:scale-105 btn-press" style={{background:'linear-gradient(135deg,#2578A4,#97C3EE)'}}>
          ...
          {lotteryActive && (
            <div className="p-3 rounded-xl" style={{ background: ticketQtyInCart > 0 ? 'rgba(8,145,178,0.08)' : 'rgba(8,145,178,0.03)', border: '1px solid rgba(8,145,178,0.15)' }}>
              <span className="text-sm font-bold flex items-center gap-1.5" style={{ color: '#041C33' }}><Gift className="w-4 h-4 text-cyan-600" /> Partecipa alla lotteria</span>
                          ...
                          return (
                            <button key={n} type="button" disabled={isTaken}
                              ...
                              style={isTaken
                                ...
                                : isChosen
                                  ? { background: 'linear-gradient(135deg,#2578A4,#97C3EE)', color: 'white' }
                                  : { background: 'rgba(8,145,178,0.05)', border: '1px solid rgba(8,145,178,0.15)', color: '#041C33' }}>
                              {n}
                            </button>
                          )
                        })}
          ...
          <div className="border-t border-cyan-100 pt-3 space-y-1.5">
            ...
            <div className="flex justify-between font-bold pt-1"><span style={{color:'#041C33'}}>Totale</span><span className="text-xl" style={{color:'#2578A4'}}>€{total.toFixed(2)}</span></div>
          </div>
          <CodBanner />
          ...
          <div className="space-y-2.5">
            <label className="flex items-center gap-2.5 p-3 rounded-xl cursor-pointer select-none" style={{ background: wantsDelivery ? 'rgba(8,145,178,0.06)' : 'rgba(8,145,178,0.02)', border: '1px solid rgba(8,145,178,0.15)' }}>
              ...
              <span className="flex items-center gap-2 text-sm font-medium" style={{ color: '#041C33' }}>
            ...
            {wantsDelivery && (
              <input type="text" placeholder="Indirizzo di consegna (via, civico, città)" value={address} onChange={e => setAddress(e.target.value)}
                className="w-full h-11 px-4 rounded-xl text-base outline-none" style={{ background: 'rgba(8,145,178,0.05)', border: '1px solid rgba(8,145,178,0.15)', color: '#041C33' }} />
            )}
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <button type="submit" disabled={submitting} className="w-full py-3.5 rounded-xl font-bold text-white transition-all hover:scale-[1.02] btn-press disabled:opacity-60" style={{background:'linear-gradient(135deg,#2578A4,#97C3EE)',boxShadow:'0 8px 20px rgba(8,145,178,0.3)'}}>
