    ...
    <>
      ...
      {showStickyCart && cartCount > 0 && !isOpen && (
        <Link
          ...
          style={{ background: '#041C33' }}
          aria-label="Vai al carrello"
        >
          <ShoppingBag className="w-6 h-6" />
          <span className="absolute -top-1 -right-1 w-5 h-5 text-white text-xs rounded-full flex items-center justify-center font-bold" style={{ background: 'linear-gradient(135deg,#2578A4,#97C3EE)' }}>
            {cartCount}
          </span>
        </Link>
      )}
      ...
      {menuOpen && !chatOpen && !pointsOpen && (
        <div className={`fixed ${menuOffsetClass} right-5 z-40 flex flex-col items-end gap-3`}>
          <a href={SOCIAL_LINKS.instagram} target="_blank" rel="noopener noreferrer"
            ...
            Instagram
            <span className="w-9 h-9 rounded-full flex items-center justify-center text-white shrink-0" style={{ background: 'linear-gradient(135deg,#97C3EE,#d946ef,#D86183)' }}>
          ...
          <button onClick={openChat}
            className="flex items-center gap-2.5 pl-4 pr-2 py-2 rounded-full shadow-lg bg-white text-sm font-medium text-slate-700 transition-transform hover:scale-105">
            Scrivici in chat
            <span className="w-9 h-9 rounded-full flex items-center justify-center text-white shrink-0" style={{ background: 'linear-gradient(135deg,#2578A4,#97C3EE)' }}>
      ...
      {chatOpen && (
        <div className={`fixed ${menuOffsetClass} right-5 z-40 w-[90vw] max-w-sm h-[480px] max-h-[70vh] bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-slate-100`}>
          <div className="px-4 py-3 text-white font-semibold flex items-center gap-2" style={{ background: 'linear-gradient(135deg,#2578A4,#97C3EE)' }}>
                ...
                {messages.map(m => (
                  <div key={m.id} className={`flex ${m.sender === 'customer' ? 'justify-end' : 'justify-start'}`}>
                    <div
                      ...
                      style={m.sender === 'customer' ? { background: 'linear-gradient(135deg,#2578A4,#97C3EE)' } : undefined}
                    >
                      {m.message}
                    </div>
                  </div>
