...
export default function PreferitiPage() {
  ...
  const handleAdd = (p: Product) => {
    ...
    toast.success(`${p.name} aggiunto al carrello!`, {
      style: { background: '#D2E6FB', border: '1px solid #2578A4', color: '#0B3C65' }
    })
  }
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-1 flex items-center gap-2" style={{ color: '#041C33' }}>
          ...
          {products.map(p => {
            ...
            return (
              <div key={p.id} className="rounded-2xl overflow-hidden animate-fade-in-up"
                ...
                <div className="relative aspect-square cursor-pointer" style={{ background: 'linear-gradient(135deg, #EDF5FD, #D2E6FB)' }}
                ...
                <div className="p-3">
                  <h3 className="font-semibold text-sm line-clamp-2 mb-2 leading-snug cursor-pointer" style={{ color: '#041C33' }}
                    onClick={() => openDetail(p.id)}>
                    {p.name}
                  </h3>
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-base" style={{ color: '#2578A4' }}>€{p.price.toFixed(2)}</span>
                    <button onClick={() => handleAdd(p)} disabled={p.torna_presto}
                      ...
                      style={p.torna_presto
                        ...
                        : { background: 'linear-gradient(135deg, #2578A4, #97C3EE)', boxShadow: '0 2px 8px rgba(8,145,178,0.3)' }}>
