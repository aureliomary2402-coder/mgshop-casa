      ...
      {[{v:t.days,l:'Giorni'},{v:t.hours,l:'Ore'},{v:t.minutes,l:'Min'},{v:t.seconds,l:'Sec'}].map(({v,l},i)=>(
        <div key={l} className="flex items-center gap-3">
          <div className="text-center">
            <div className="w-14 h-14 rounded-xl flex items-center justify-center font-bold text-2xl text-white" style={{background:'linear-gradient(135deg,#2578A4,#97C3EE)',boxShadow:'0 4px 16px rgba(8,145,178,0.3)'}}>{String(v).padStart(2,'0')}</div>
            <p className="text-xs mt-1" style={{ color: '#041C3399' }}>{l}</p>
          </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex flex-col">
            {hasDiscount ? (
              ...
              <span className="font-bold text-lg" style={{ color: '#2578A4' }}>€{salePrice.toFixed(2)}</span>
            )}
          </div>
          <button onClick={handleAdd}
            ...
            style={product.torna_presto
              ...
              : { background: added ? 'linear-gradient(135deg,#16a34a,#22c55e)' : 'linear-gradient(135deg,#2578A4,#97C3EE)', boxShadow: '0 4px 12px rgba(8,145,178,0.3)' }}>
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center p-0 sm:p-4" style={{ background: 'rgba(12,43,54,0.5)' }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} className="bg-white w-full sm:max-w-md sm:rounded-3xl rounded-t-3xl overflow-hidden max-h-[90vh] flex flex-col animate-slide-in-right">
        <div className="relative shrink-0" style={{ height: '200px', background: 'linear-gradient(135deg,#EDF5FD,#D2E6FB)' }}>
          {product.cover_image
        ...
        <div className="p-5 overflow-y-auto flex-1 space-y-4">
          <h2 className="text-xl font-bold" style={{color:'#041C33'}}>{product.name}</h2>
          <div className="flex items-baseline gap-2">
            {hasDiscount && <span className="text-sm text-slate-400 line-through">€{product.price.toFixed(2)}</span>}
            <span className="text-2xl font-bold" style={{ color: hasDiscount ? '#dc2626' : '#2578A4' }}>€{salePrice.toFixed(2)}</span>
          </div>
          {product.description && (
          ...
          <button onClick={handleAdd}
            ...
            style={product.torna_presto
              ? { background: '#94a3b8', boxShadow: 'none' }
              : { background: added ? 'linear-gradient(135deg,#16a34a,#22c55e)' : 'linear-gradient(135deg,#2578A4,#97C3EE)', boxShadow: '0 8px 20px rgba(8,145,178,0.3)' }}>
  ...
  if(!promo||!promo.is_active) return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: 'linear-gradient(180deg,#EDF5FD 0%,#F7FBFF 45%,#ffffff 100%)' }}>
      <div className="text-center max-w-md">
        ...
        <h1 className="text-2xl font-bold mb-2" style={{color:'#041C33'}}>Nessuna promo attiva</h1>
        <p className="text-slate-400 mb-8">Torna presto per le nostre offerte!</p>
        <Link href="/shop" className="inline-flex items-center gap-2 font-bold px-8 py-4 rounded-2xl text-white" style={{background:'linear-gradient(135deg,#2578A4,#97C3EE)'}}><ArrowLeft className="w-4 h-4"/> Vai al negozio</Link>
      </div>
    </div>
  )
    ...
    <div className="min-h-screen" style={{ background: 'linear-gradient(180deg,#EDF5FD 0%,#F7FBFF 45%,#ffffff 100%)' }}>
      ...
      <div className="relative overflow-hidden theme-hero">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 py-12 text-center">
          ...
          {promo.badge_text&&<div className="inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-sm font-medium mb-4" style={{background:'rgba(8,145,178,0.1)', color: '#2578A4', border:'1px solid rgba(8,145,178,0.2)'}}><Tag className="w-4 h-4"/> {promo.badge_text}</div>}
          <h1 className="text-4xl md:text-5xl font-extrabold mb-3" style={{ color: '#041C33' }}>{promo.title}</h1>
          {promo.subtitle&&<p className="text-lg mb-6" style={{ color: '#041C33cc' }}>{promo.subtitle}</p>}
          {promo.expires_at&&<div className="mb-4"><div className="flex items-center justify-center gap-2 text-sm mb-3" style={{ color: '#2578A4' }}><Clock className="w-4 h-4"/> Offerta valida ancora per:</div><Countdown expiresAt={promo.expires_at}/></div>}
        </div>
      </div>
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-4 py-10 space-y-10">
          ...
          {displayItems.length > 0 && (
            <Reveal delay={100}>
              <h2 className="text-2xl font-bold mb-6" style={{color:'#041C33'}}>Prodotti in promozione</h2>
