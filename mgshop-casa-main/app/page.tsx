...
export default function WelcomePage() {
  ...
  const BUBBLES: BubbleDef[] = [
    { key: 'promo', label: 'Promozioni', sub: 'Scopri le offerte della settimana', icon: Tag, color: '#D86183', badge: promoActive, mobileArea: 'promo', desktopArea: 'promo', action: { type: 'link', href: '/promo' } },
    { key: 'promobox', label: 'Promo Box', sub: 'Più prodotti, più convenienza', icon: Package, color: '#2578A4', mobileArea: 'promobox', desktopArea: 'promobox', action: { type: 'soon' } },
    { key: 'lotteria', label: 'Lotteria', sub: 'Scegli il numero e prova a vincere', icon: Ticket, color: '#0B3C65', badge: lotteryActive, mobileArea: 'lotteria', desktopArea: 'lotteria', action: { type: 'link', href: '/lotteria' } },
    { key: 'punti', label: 'Punti', sub: 'Accumula punti e ottieni premi', icon: Star, color: '#97C3EE', mobileArea: 'punti', desktopArea: 'punti', action: { type: 'points' } },
    { key: 'negozio', label: 'Negozio', sub: 'Scopri tutti i prodotti', icon: ShoppingBag, color: '#D86183', big: true, mobileArea: 'negozio', desktopArea: 'negozio', action: { type: 'link', href: '/shop' } },
    { key: 'volantino', label: 'Volantino', sub: 'Sfoglia le nostre offerte', icon: Newspaper, color: '#2578A4', badge: volantinoActive, mobileArea: 'volantino', desktopArea: 'volantino', action: { type: 'link', href: '/volantino' } },
    { key: 'zona', label: 'Zone di Consegna', sub: 'Scopri se consegniamo da te', icon: MapPin, color: '#2578A4', mobileArea: 'zona', desktopArea: 'zona', action: { type: 'link', href: '/consegne' } },
    { key: 'contatti', label: 'Contatti', sub: 'Siamo qui per te', icon: Phone, color: '#0B3C65', mobileArea: 'contatti', desktopArea: 'contatti', action: { type: 'chat' } },
    { key: 'account', label: 'Il Mio Account', sub: 'Ordini, punti e premi', icon: User, color: '#2578A4', mobileArea: 'account', desktopArea: 'account', action: { type: 'soon' } },
  ]
  ...
  function renderBubble(b: BubbleDef, variant: 'mobile' | 'desktop') {
    ...
    const content = (
      <>
        ...
        <span className={`relative z-10 ${labelSize}`} style={{ color: '#041C33' }}>{b.label}</span>
  ...
  return (
    <div className="min-h-screen" style={{ background: 'linear-gradient(180deg,#EDF5FD 0%,#F7FBFF 45%,#ffffff 100%)' }}>
      ...
      <section className="relative overflow-hidden"
        style={{ background: 'linear-gradient(160deg,#B5D6F6 0%,#EDF5FD 55%,#ffffff 100%)' }}>
        ...
        <div className={`relative z-10 max-w-7xl mx-auto px-6 pt-10 pb-10 lg:py-16 lg:grid lg:grid-cols-2 lg:gap-12 lg:items-center ${mounted ? 'animate-fade-in-up' : 'opacity-0'}`}>
          ...
          <div className="text-center lg:text-left">
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight leading-[1.08]">
              <span style={{ color: '#041C33' }}>TUTTO PER<br />LA TUA CASA.</span><br />
              <span style={{ color: '#D86183' }}>CONSEGNATO<br />A CASA TUA.</span>
            </h1>
            <p className="mt-5 text-sm sm:text-base leading-relaxed max-w-xl mx-auto lg:mx-0" style={{ color: '#041C33cc' }}>
            ...
            <div className="mt-6 grid grid-cols-2 gap-3 rounded-2xl p-4 text-left lg:hidden"
              ...
              {HERO_ADVANTAGES.map((a, i) => (
                <div key={i} className="flex items-center gap-2.5">
                  ...
                  <span className="text-xs font-medium leading-snug" style={{ color: '#041C33' }}>{a.label}</span>
                </div>
              ))}
            </div>
            <div className="hidden lg:flex lg:flex-col gap-3.5 mt-7">
              {HERO_ADVANTAGES.map((a, i) => (
                <div key={i} className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-medium" style={{ color: '#041C33' }}>{a.label}</span>
                </div>
              ))}
            </div>
            ...
            <Link href="/promo"
              ...
              style={{ background: 'linear-gradient(135deg,#D86183,#E58DA7)', boxShadow: '0 8px 24px rgba(219,39,119,0.3)' }}>
            ...
            {BOTTOM_ADVANTAGES.map((a, i) => (
              <div key={i}
                ...
                <span className="w-9 h-9 sm:w-10 sm:h-10 rounded-full flex items-center justify-center shrink-0"
                  style={{ background: 'rgba(8,145,178,0.08)', color: '#2578A4' }}>
                  <a.icon className="w-4 h-4 sm:w-5 sm:h-5" />
                </span>
                <div>
                  <p className="text-[11px] sm:text-xs font-bold tracking-wide" style={{ color: '#041C33' }}>{a.title}</p>
