    ...
    <div className="min-h-screen" style={{ background: 'linear-gradient(180deg,#EDF5FD 0%,#F7FBFF 45%,#ffffff 100%)' }}>
      ...
      <div className="relative overflow-hidden theme-hero">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 py-12 text-center">
          <Link href="/shop" className="inline-flex items-center gap-2 text-sm transition-colors mb-6" style={{ color: '#2578A4' }}>
            <ArrowLeft className="w-4 h-4" /> Negozio
          </Link>
          <PageHeroIcon icon={MapPin} color="#2578A4" />
          <div className="inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-sm font-medium mb-4"
            style={{ background: 'rgba(8,145,178,0.1)', color: '#2578A4', border: '1px solid rgba(8,145,178,0.2)' }}>
            <Truck className="w-4 h-4" /> Zone di consegna
          </div>
          <h1 className="text-4xl md:text-5xl font-extrabold mb-3" style={{ color: '#041C33' }}>Dove consegniamo</h1>
          <p className="text-lg max-w-xl mx-auto" style={{ color: '#041C33cc' }}>
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-4 py-10 space-y-6">
              ...
              {ZONES.map(z => (
                <div key={z.title} className="glass-card rounded-2xl p-6"
                  ...
                  <div className="w-11 h-11 rounded-xl flex items-center justify-center mb-4"
                    ...
                    <MapPin className="w-5 h-5" style={{ color: z.highlight ? '#16a34a' : '#2578A4' }} />
                  </div>
                  <h3 className="text-lg font-bold mb-1" style={{ color: '#041C33' }}>{z.title}</h3>
                  <p className="text-xl font-extrabold mb-2" style={{ color: z.highlight ? '#16a34a' : '#2578A4' }}>{z.price}</p>
          ...
          <Reveal delay={200} className="text-center pt-4">
            <Link href="/shop" className="inline-flex items-center gap-2 font-bold px-10 py-4 rounded-2xl text-white"
              style={{ background: 'linear-gradient(135deg,#2578A4,#97C3EE)', boxShadow: '0 12px 32px rgba(8,145,178,0.35)' }}>
