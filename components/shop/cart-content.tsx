...
export function CartContent({ scope = 'shop' }: { scope?: string }) {
  ...
  if (submitted) return (
    <div className="max-w-md mx-auto px-4 py-20 text-center animate-scale-in">
      ...
      <p className="text-sm text-amber-700 font-medium mb-8">🎁 Riceverai i tuoi punti fedeltà via WhatsApp!</p>
  ...
  if (items.length === 0) return (
    <div className="max-w-md mx-auto px-4 py-20 text-center animate-scale-in">
      ...
      <Link href="/shop" className="inline-flex items-center gap-2 font-bold px-8 py-4 rounded-2xl text-white btn-press" style={{background:'linear-gradient(135deg,#d97706,#f59e0b)'}}>Vai ai prodotti</Link>
    </div>
  )
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="grid md:grid-cols-3 gap-6">
          ...
          {items.map(({product,quantity},i) => (
            <div key={product.id} className="flex gap-4 rounded-2xl p-4 animate-fade-in-up"
              ...
              <div className="w-20 h-20 rounded-xl overflow-hidden shrink-0" style={{background:'linear-gradient(135deg,#faf7f2,#fef3c7)'}}>
                {product.cover_image ? <img src={product.cover_image} alt={product.name} className="w-full h-full object-cover"/> : <div className="w-full h-full flex items-center justify-center"><ImageIcon className="w-8 h-8" style={{color:'rgba(217,119,6,0.3)'}}/></div>}
              </div>
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-2">
                  <button onClick={() => updateQuantity(product.id,quantity-1)} className="w-7 h-7 rounded-full flex items-center justify-center transition-all hover:scale-110 btn-press" style={{border:'1px solid rgba(217,119,6,0.2)',background:'rgba(217,119,6,0.04)'}}><Minus className="w-3 h-3" style={{color:'#92400e'}}/></button>
                  <span className="w-6 text-center text-sm font-bold" style={{color:'#1a0800'}}>{quantity}</span>
                  <button onClick={() => updateQuantity(product.id,quantity+1)} className="w-7 h-7 rounded-full flex items-center justify-center transition-all hover:scale-110 btn-press" style={{border:'1px solid rgba(217,119,6,0.2)',background:'rgba(217,119,6,0.04)'}}><Plus className="w-3 h-3" style={{color:'#92400e'}}/></button>
        ...
        <div className="rounded-2xl p-5 h-fit sticky top-20 animate-slide-in-right space-y-4" style={{background:'white',border:'1px solid rgba(217,119,6,0.1)',boxShadow:'0 8px 24px rgba(217,119,6,0.08)'}}>
                ...
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    ...
                    <input value={couponInput} onChange={e => setCouponInput(e.target.value.toUpperCase())} onKeyDown={e => e.key==='Enter'&&handleApplyCoupon()} placeholder="Codice coupon"
                      className="w-full h-10 pl-9 pr-3 rounded-xl text-sm font-mono outline-none" style={{background:'rgba(217,119,6,0.05)',border:'1px solid rgba(217,119,6,0.15)',color:'#1a0800'}}/>
                  </div>
                  <button onClick={handleApplyCoupon} disabled={couponLoading||!couponInput.trim()} className="px-3 py-2 rounded-xl text-sm font-medium text-white disabled:opacity-50 transition-all hover:scale-105 btn-press" style={{background:'linear-gradient(135deg,#d97706,#f59e0b)'}}>
          ...
          <div className="border-t border-amber-100 pt-3 space-y-1.5">
            <div className="flex justify-between text-sm text-stone-500"><span>Subtotale</span><span>€{subtotal.toFixed(2)}</span></div>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <button type="submit" disabled={submitting} className="w-full py-3.5 rounded-xl font-bold text-white transition-all hover:scale-[1.02] btn-press disabled:opacity-60" style={{background:'linear-gradient(135deg,#d97706,#f59e0b)',boxShadow:'0 8px 20px rgba(217,119,6,0.3)'}}>
