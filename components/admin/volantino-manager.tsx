...
export function VolantinoManager() {
  const [list, setList] = useState<VolantinoRecord[]>([])
  const [selectedId, setSelectedId] = useState<string | null>(null)
  const [loading, setLoading] = useState(true)
  const [creating, setCreating] = useState(false)

  // Campi del volantino attualmente selezionato
  ...
  const selectVolantino = (v: VolantinoRecord) => {
    setSelectedId(v.id)
  ...
  const handleCreate = async () => {
    ...
    try {
      const res = await fetch('/api/admin/volantino', {
        ...
        body: JSON.stringify({ title: 'Nuovo volantino' })
      })
      if (!res.ok) { setError('Errore nella creazione'); return }
      const created: VolantinoRecord = await res.json()
      const arr = await loadList()
      const fresh = arr.find(v => v.id === created.id) || created
      selectVolantino(fresh)
    } catch {
  ...
  const handleDelete = async () => {
    if (!selectedId) return
    if (!confirm('Eliminare definitivamente questo volantino?')) return
    setDeleting(true)
    const res = await fetch(`/api/admin/volantino/${selectedId}`, { method: 'DELETE' })
    setDeleting(false)
    if (!res.ok) { setError('Errore eliminazione'); return }
    const arr = await loadList()
    if (arr.length > 0) selectVolantino(arr[0])
    else {
      setSelectedId(null)
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <label className="text-xs font-medium text-slate-500">I tuoi volantini ({list.length})</label>
        <button onClick={handleCreate} disabled={creating}
          className="flex items-center gap-1 text-xs text-cyan-700 font-medium px-3 py-1.5 rounded-lg border border-cyan-200 hover:bg-cyan-50 transition-colors disabled:opacity-50">
          <Plus className="w-3.5 h-3.5" /> {creating ? 'Creazione...' : 'Nuovo volantino'}
        </button>
      </div>

      {list.length === 0 ? (
        <div className="text-center py-8 text-slate-400 text-sm border border-dashed border-slate-200 rounded-2xl">
          Nessun volantino ancora. Creane uno per iniziare.
        </div>
      ) : (
        <div className="flex flex-wrap gap-2">
          {list.map(v => (
            <button key={v.id} onClick={() => selectVolantino(v)}
              ...
              {v.title || 'Senza titolo'}
              <span className={`w-1.5 h-1.5 rounded-full ${v.is_active ? 'bg-emerald-400' : 'bg-slate-300'}`} />
            </button>
          ))}
        </div>
      )}
        ...
        <div className="space-y-5 pt-2 border-t border-slate-100">
          ...
          <div className="flex items-center justify-between p-4 rounded-2xl border"
            ...
            <div className="flex items-center gap-3">
              {isActive && (
                <Link href={`/volantino/${slug || selectedId}`} target="_blank" className="flex items-center gap-1.5 text-xs text-cyan-700 font-medium px-3 py-1.5 rounded-lg border border-cyan-200 hover:bg-cyan-50">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input value={slug} onChange={e => setSlug(e.target.value)} placeholder="Es. offerte-agosto" />
              <p className="text-[11px] text-slate-400 mt-1">Se lo lasci vuoto viene usato un link automatico.</p>
            </div>
          ...
          <div className="flex gap-2">
            <Button onClick={handleSave} disabled={saving} className="flex-1 gap-2 bg-cyan-600 hover:bg-cyan-700 text-base py-6">
              <Save className="w-5 h-5" />
              {saved ? 'Salvato!' : saving ? 'Salvataggio...' : 'Salva modifiche'}
            </Button>
            <button onClick={handleDelete} disabled={deleting}
              className="px-4 rounded-lg border border-red-200 text-red-500 hover:bg-red-50 transition-colors disabled:opacity-50"
