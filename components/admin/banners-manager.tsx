...
export function BannersManager() {
  ...
  if (creating || editing) return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="font-semibold text-stone-800">{creating ? 'Nuovo banner' : 'Modifica banner'}</h2>
      ...
      <div className="space-y-3">
        ...
        <div>
          <label className="text-xs font-medium text-stone-500 mb-1 block">Link (opzionale)</label>
          <Input value={form.link} onChange={e => setForm(f => ({ ...f, link: e.target.value }))} placeholder="https://..." />
        </div>
        <div>
          <label className="text-xs font-medium text-stone-500 mb-1 block">Ordine</label>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="font-semibold text-stone-800">Banner ({banners.length})</h2>
      ...
      <div className="space-y-2">
        {banners.map(b => (
          <div key={b.id} className="flex items-center gap-3 bg-white border border-stone-100 rounded-xl p-3 shadow-sm">
            <div className="w-16 h-10 rounded-lg bg-stone-50 overflow-hidden shrink-0">
              <img src={b.image_url} alt="" className="w-full h-full object-cover" />
            </div>
            <div className="flex-1 min-w-0">
              <p className="font-medium text-sm text-stone-800 truncate">{b.title || 'Banner senza titolo'}</p>
              <p className="text-xs text-stone-400">Ordine: {b.display_order}</p>
            </div>
            <div className="flex items-center gap-1 shrink-0">
              <button onClick={() => handleToggle(b)}>
                {b.is_active ? <ToggleRight className="w-6 h-6 text-amber-600" /> : <ToggleLeft className="w-6 h-6 text-stone-300" />}
              </button>
              <button onClick={() => openEdit(b)} className="p-1.5 hover:bg-stone-100 rounded-lg transition-colors">
        ...
        {banners.length === 0 && <p className="text-center py-6 text-stone-400 text-sm">Nessun banner</p>}
      </div>
    </div>
  )
}
