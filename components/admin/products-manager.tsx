...
import type { Product, Category, CustomizationOption, CustomizationChoice } from '@/lib/types'
...
export function ProductsManager() {
  ...
  const [form, setForm] = useState({ name: '', description: '', price: '', category_id: '', cover_image: '', card_image: '', is_active: true, stock: '', torna_presto: false, is_customizable: false, customization_options: [] as CustomizationOption[], customization_note: '', keywords: '' })
  ...
  const openCreate = () => {
    setForm({ name: '', description: '', price: '', category_id: '', cover_image: '', card_image: '', is_active: true, stock: '', torna_presto: false, is_customizable: false, customization_options: [], customization_note: '', keywords: '' })
  ...
  if (creating || editing) return (
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        ...
        <div>
          <label className="text-xs font-medium text-slate-500 mb-1 block">Parole chiave per la ricerca</label>
          <Input value={form.keywords} onChange={e => setForm(f => ({ ...f, keywords: e.target.value }))}
            placeholder="es. sapone liquido, sapone mani, detersivo bucato" />
          <p className="text-xs text-slate-400 mt-1">Termini che i clienti potrebbero cercare anche se diversi dal nome del prodotto, separati da virgola.</p>
        </div>
