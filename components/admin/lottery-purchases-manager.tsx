        ...
        {rounds.map(round => (
          <div key={round.round_id} className="space-y-2">
            <div className="flex items-center gap-2 pb-1 border-b border-slate-200">
              <span className={`text-xs font-bold px-2 py-1 rounded-full ${round.is_current ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-500'}`}>
                {round.label}
              </span>
              <span className="text-xs text-slate-400">{round.ticket_count} bigliett{round.ticket_count > 1 ? 'i' : 'o'}</span>
            </div>
            {purchases.filter(p => p.round_id === round.round_id).map(p => (
