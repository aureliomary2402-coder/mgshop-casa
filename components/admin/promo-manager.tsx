      ...
      <div className="flex items-center justify-between p-4 rounded-2xl border"
        ...
        <div>
          ...
          <p className="text-xs mt-0.5">
            {isActive ? <span className="text-amber-600 font-medium">Attiva — ricordati di salvare!</span> : <span className="text-stone-400">Disattivata — ricordati di salvare!</span>}
          </p>
        </div>
        <div className="flex items-center gap-3">
          {isActive && (
            <Link href="/promo" target="_blank" className="flex items-center gap-1.5 text-xs text-amber-700 font-medium px-3 py-1.5 rounded-lg border border-amber-200 hover:bg-amber-50">
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <textarea value={content} onChange={e => setContent(e.target.value)}
            className="w-full border border-stone-200 rounded-lg p-3 text-sm resize-none h-28 focus:outline-none focus:ring-2 focus:ring-amber-500"
        ...
        <div>
          ...
          {imageUrl && (
            <div className="mt-2 relative">
              <div className="rounded-xl overflow-hidden h-28 bg-stone-100"><img src={imageUrl} alt="preview" className="w-full h-full object-cover" /></div>
        ...
        <div>
          <label className="text-xs font-medium text-stone-500 mb-1 flex items-center gap-1"><Clock className="w-3.5 h-3.5" /> Scadenza</label>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="text-xs font-medium text-stone-500">Prodotti in promozione ({featuredIds.length})</label>
          ...
          {featuredProducts.length > 0 && (
            <div className="space-y-2 mb-3">
              {featuredProducts.map(p => (
                <div key={p.id} className="flex items-center gap-3 p-2 rounded-xl border border-amber-100 bg-amber-50">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-amber-700 font-bold">euro{p.price.toFixed(2)}</p>
                  </div>
              ...
              <div className="max-h-60 overflow-y-auto">
                {filteredProducts.map(p => (
                  <button key={p.id} onClick={() => toggleProduct(p.id)}
                    className={`w-full flex items-center gap-3 p-3 hover:bg-stone-50 transition-colors text-left border-b border-stone-50 ${featuredIds.includes(p.id) ? 'bg-amber-50' : ''}`}>
                    {p.cover_image
                    ...
                    {featuredIds.includes(p.id) && <span className="text-xs text-amber-600 font-bold shrink-0">Selezionato</span>}
                  </button>
                ))}
              </div>
