...
interface PromoItem {
  ...
  // Stessa personalizzazione dei prodotti veri del negozio (scelte con un
  // prezzo diverso ciascuna): solo per i prodotti creati apposta qui nella
  // promo (isCustom), non per quelli presi dal catalogo (che hanno già le
  // proprie opzioni impostate su /shop).
...
export function PromoManager() {
  ...
  // Attiva/disattiva la personalizzazione (scelte con prezzo diverso) su un
  // prodotto creato apposta per la promo, e aggiorna le sue opzioni/nota:
  // stesso meccanismo identico ai prodotti veri del negozio.
  const updateIsCustomizable = (id: string, value: boolean) => {
    setItems(prev => prev.map(i => i.id === id ? { ...i, is_customizable: value } : i))
  }
  const updateCustomizationOptions = (id: string, options: CustomizationOption[]) => {
    setItems(prev => prev.map(i => i.id === id ? { ...i, customization_options: options } : i))
  }
  const updateCustomizationNote = (id: string, note: string) => {
    setItems(prev => prev.map(i => i.id === id ? { ...i, customization_note: note } : i))
  }
            ...
            <div className="space-y-2 mb-3">
              {itemProducts.map(({ item, name, image, originalPrice, isCustom }) => (
                <div key={item.id} className="p-2.5 rounded-xl border border-cyan-100 bg-cyan-50 space-y-2">
                  ...
                  {isCustom && (
                    <div className="flex items-center gap-2 p-2 rounded-lg border bg-white border-fuchsia-100">
                      <button onClick={() => updateIsCustomizable(item.id, !item.is_customizable)} className="shrink-0">
                        {item.is_customizable ? <ToggleRight className="w-7 h-7 text-fuchsia-600" /> : <ToggleLeft className="w-7 h-7 text-slate-400" />}
                      </button>
                      <div>
                        <p className="text-xs font-semibold text-slate-700">Prodotto personalizzabile</p>
                        <p className="text-[10px] text-slate-400">Il cliente sceglie tra più opzioni, ognuna con un prezzo diverso — stesso funzionamento del negozio/volantino</p>
                      </div>
                    </div>
                  )}
                  {isCustom && item.is_customizable && (
                    <div className="rounded-xl border border-fuchsia-200 bg-fuchsia-50/40 p-3 space-y-2.5">
                      <div>
                        <label className="text-[11px] text-fuchsia-700 block mb-0.5">Testo introduttivo (facoltativo)</label>
                        <Input value={item.customization_note || ''} onChange={e => updateCustomizationNote(item.id, e.target.value)}
                          placeholder="Es. Scegli la quantità che preferisci" className="bg-white" />
                      </div>
                      <CustomizationOptionsEditor
                        options={item.customization_options || []}
                        onChange={opts => updateCustomizationOptions(item.id, opts)}
                      />
                      <p className="text-[11px] text-slate-400">Il prezzo promo qui sopra viene usato solo se nessuna scelta ha un prezzo proprio.</p>
                    </div>
                  )}
                </div>
              ))}
