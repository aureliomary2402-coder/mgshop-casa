...
function Countdown({ expiresAt }: { expiresAt: string }) {
  ...
  if(expired) return <div className="text-center text-slate-200/40 text-sm">Offerta scaduta</div>
  return (
    <div className="flex items-center justify-center gap-3">
      {[{v:t.days,l:'Giorni'},{v:t.hours,l:'Ore'},{v:t.minutes,l:'Min'},{v:t.seconds,l:'Sec'}].map(({v,l},i)=>(
        <div key={l} className="flex items-center gap-3">
          <div className="text-center">
            <div className="w-14 h-14 rounded-xl flex items-center justify-center font-bold text-2xl text-white" style={{background:'linear-gradient(135deg,#0891b2,#06b6d4)',boxShadow:'0 4px 16px rgba(100,116,139,0.3)'}}>{String(v).padStart(2,'0')}</div>
            <p className="text-xs text-slate-200/50 mt-1">{l}</p>
          </div>
...
function PromoProductCard({ product, coupon }: { product: Product; coupon: Coupon | null }) {
  ...
  return (
    <div className="bg-white rounded-2xl overflow-hidden shadow-sm hover:-translate-y-1 transition-all group"
      style={{ border: '1px solid rgba(100,116,139,0.1)', boxShadow: '0 4px 20px rgba(100,116,139,0.08)' }}>
      <Link href={`/prodotto/${product.id}`}>
        <div className="aspect-square overflow-hidden relative" style={{ background: 'linear-gradient(135deg,#fafafa,#cffafe)' }}>
          {product.cover_image
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <button onClick={handleAdd}
            ...
            style={{ background: added ? 'linear-gradient(135deg,#16a34a,#22c55e)' : 'linear-gradient(135deg,#0891b2,#06b6d4)', boxShadow: '0 4px 12px rgba(100,116,139,0.3)' }}>
...
export default function PromoPage() {
  ...
  if(!promo||!promo.is_active) return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{background:'#fafafa'}}>
      <div className="text-center max-w-md">
        <div className="w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-6" style={{background:'rgba(100,116,139,0.08)',border:'2px dashed rgba(100,116,139,0.2)'}}><ShoppingBag className="w-12 h-12" style={{color:'rgba(100,116,139,0.4)'}}/></div>
  ...
  return (
    <div className="min-h-screen" style={{background:'#fafafa'}}>
      {/* Hero */}
      <div className="relative overflow-hidden" style={{background:'linear-gradient(135deg,#1c1c1f,#27272a)'}}>
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 py-12 text-center">
          ...
          {promo.badge_text&&<div className="inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-cyan-300 text-sm font-medium mb-4" style={{background:'rgba(100,116,139,0.15)',border:'1px solid rgba(100,116,139,0.3)'}}><Tag className="w-4 h-4"/> {promo.badge_text}</div>}
          <h1 className="text-4xl md:text-5xl font-extrabold text-white mb-3">{promo.title}</h1>
          {promo.subtitle&&<p className="text-lg text-slate-200/60 mb-6">{promo.subtitle}</p>}
          {promo.expires_at&&<div className="mb-4"><div className="flex items-center justify-center gap-2 text-slate-400/60 text-sm mb-3"><Clock className="w-4 h-4"/> Offerta valida ancora per:</div><Countdown expiresAt={promo.expires_at}/></div>}
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-10 space-y-10">
        {(promo.image_url||promo.content)&&(
          <Reveal className={`grid gap-6 ${promo.image_url&&promo.content?'md:grid-cols-2':''}`}>
            {promo.image_url&&<div className="rounded-2xl overflow-hidden aspect-video" style={{boxShadow:'0 16px 40px rgba(100,116,139,0.12)'}}><img src={promo.image_url} alt="Promo" className="w-full h-full object-cover"/></div>}
            {promo.content&&<div className="flex items-center"><div className="bg-white rounded-2xl p-6 w-full" style={{border:'1px solid rgba(100,116,139,0.1)'}}><p className="text-slate-600 leading-relaxed whitespace-pre-line">{promo.content}</p></div></div>}
          </Reveal>
        )}
