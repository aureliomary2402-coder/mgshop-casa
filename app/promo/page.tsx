...
function Countdown({ expiresAt }: { expiresAt: string }) {
  ...
  if(expired) return <div className="text-center text-amber-200/40 text-sm">Offerta scaduta</div>
  return (
      ...
      {[{v:t.days,l:'Giorni'},{v:t.hours,l:'Ore'},{v:t.minutes,l:'Min'},{v:t.seconds,l:'Sec'}].map(({v,l},i)=>(
        <div key={l} className="flex items-center gap-3">
          <div className="text-center">
            ...
            <p className="text-xs text-amber-200/50 mt-1">{l}</p>
          </div>
  ...
  if(!promo||!promo.is_active) return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{background:'#faf7f2'}}>
      <div className="text-center max-w-md">
        ...
        <Link href="/shop" className="inline-flex items-center gap-2 font-bold px-8 py-4 rounded-2xl text-white" style={{background:'linear-gradient(135deg,#d97706,#f59e0b)'}}><ArrowLeft className="w-4 h-4"/> Vai al negozio</Link>
      </div>
    </div>
  )
    ...
    <div className="min-h-screen" style={{background:'#faf7f2'}}>
      ...
      <div className="relative overflow-hidden" style={{background:'linear-gradient(135deg,#1a0800,#2d1500)'}}>
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 py-12 text-center">
          ...
          {promo.badge_text&&<div className="inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-amber-300 text-sm font-medium mb-4" style={{background:'rgba(217,119,6,0.15)',border:'1px solid rgba(217,119,6,0.3)'}}><Tag className="w-4 h-4"/> {promo.badge_text}</div>}
          <h1 className="text-4xl md:text-5xl font-extrabold text-white mb-3">{promo.title}</h1>
          {promo.subtitle&&<p className="text-lg text-amber-200/60 mb-6">{promo.subtitle}</p>}
          {promo.expires_at&&<div className="mb-4"><div className="flex items-center justify-center gap-2 text-amber-400/60 text-sm mb-3"><Clock className="w-4 h-4"/> Offerta valida ancora per:</div><Countdown expiresAt={promo.expires_at}/></div>}
        </div>
      </div>
