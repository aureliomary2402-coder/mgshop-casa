...
export async function POST(request: NextRequest) {
  try {
    ...
    if (realItems.length > 0) {
      const orderItems = realItems.map((item: { product: { id: string; name: string; price: number; card_image?: string | null; cover_image?: string | null }; quantity: number; customization?: { option_id: string; label: string; value: string; price?: number }[]; unitPrice?: number; source?: string }) => ({
        order_id: order.id,
        ...
        // Pagina da cui il cliente ha aggiunto il prodotto (negozio,
        // volantino o promo), utile all'admin per orientarsi tra ordini
        // con molti prodotti simili.
        source: item.source || null,
        // Foto del prodotto "fotografata" al momento dell'ordine: resta
        // valida anche se in seguito il prodotto cambia o viene rimosso.
        product_image: item.product.card_image || item.product.cover_image || null,
      }))
