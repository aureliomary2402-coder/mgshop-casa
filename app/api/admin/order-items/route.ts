...
export async function POST(request: NextRequest) {
  if (!(await isAuthenticated())) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  const { order_id, product_id, product_name, product_price, quantity, product_image } = await request.json()
  const supabase = createAdminClient()
  ...
  if (existing) {
    ...
    // Aggiunto manualmente dall'admin: nessuna pagina di provenienza, ma
    // salviamo comunque la foto se disponibile per riconoscerlo a colpo d'occhio.
    await supabase.from('order_items').insert({ order_id, product_id, product_name, product_price, quantity, product_image: product_image || null })
  }
