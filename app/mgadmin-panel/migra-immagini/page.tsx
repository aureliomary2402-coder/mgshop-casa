...
export default function MigraImmaginiPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="bg-white border-b border-slate-200 px-4 h-14 flex items-center gap-3">
        <Link href="/mgadmin-panel" className="p-2 hover:bg-slate-100 rounded-lg transition-colors">
          <ArrowLeft className="w-4 h-4" />
        </Link>
        <span className="font-bold text-slate-800">Migrazione Immagini</span>
      </div>
      ...
      <div className="max-w-lg mx-auto px-4 py-8 space-y-5">
        ...
        <div className="bg-cyan-50 border border-cyan-200 rounded-xl p-4">
          <h2 className="font-semibold text-cyan-800 mb-1">Cosa fa questa pagina</h2>
        ...
        <div className="bg-white border border-slate-100 rounded-xl p-5 shadow-sm">
          ...
          {loadingStats ? (
            <p className="text-slate-400 text-sm">Controllo in corso...</p>
          ) : stats ? (
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-slate-600">Copertine prodotti</span>
                <span className="font-medium">{stats.products}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-600">Gallerie prodotti</span>
                <span className="font-medium">{stats.product_images}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-600">Banner</span>
                <span className="font-medium">{stats.banners}</span>
              </div>
              <div className="border-t border-slate-100 pt-2 flex justify-between font-bold">
                <span>Totale</span>
                <span className="text-cyan-700">{stats.total}</span>
              </div>
            </div>
          ) : (
            <p className="text-slate-400 text-sm">Errore nel caricamento</p>
          )}
        </div>
        ...
        {stats && stats.total > 0 && !result && (
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 bg-cyan-600 hover:bg-cyan-700 disabled:opacity-60 text-white font-semibold py-4 rounded-xl transition-colors"
          >
            {loading ? (
              <>
        ...
        {result && (
          <div className="bg-white border border-slate-100 rounded-xl p-5 shadow-sm space-y-4">
            <h2 className="font-semibold text-slate-800">Risultato migrazione</h2>
            <div className="grid grid-cols-3 gap-3 text-center">
              ...
              <div className="bg-slate-50 rounded-lg p-3">
                ...
                <p className="text-xs text-slate-500 mt-1">Saltate</p>
              </div>
            </div>
