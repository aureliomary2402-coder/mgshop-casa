...
cat > "app/api/admin/orders/route.ts" << 'STATEOF2'
...
cat > "app/api/admin/lottery/purchases/route.ts" << 'STATEOF2'
import { NextRequest, NextResponse } from 'next/server'
import { createAdminClient } from '@/lib/supabase/admin'
import { cookies } from 'next/headers'
...
cat > "components/admin/lottery-purchases-manager.tsx" << 'STATEOF2'
"use client"
...
mkdir -p "components/admin"
cat > "components/admin/dashboard-stats.tsx" << 'STATEOF2'
"use client"
...
git add -A
git commit -m "fix: build error (Set iteration) + incasso ordini al netto dei biglietti"
git push
